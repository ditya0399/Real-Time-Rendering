...
ALuint audiobuffer;
ALuint audiosource;
ALsizei audioalsize, audioalfreq;
ALenum audioalformat;
ALvoid *audioaldata;
ALboolean audioalloop = AL_FALSE;
...
GLfloat bendAngle = 0.0f;
GLfloat SittingFaceZ = 0.0f;
...
GLfloat walkingTranslate = -15.0f;
GLfloat walkingX = 0.0f;
GLfloat walkingY = 0.0f;
...
GLfloat AD_LeftHandX = 0.0f;
GLfloat AD_RightHandX = 0.0f;
GLfloat AD_LegY = 0.0f;
GLfloat AD_FaceY = 0.0f;
...
void UpdateSwimming();
void DemoMode();
void DemoModeSitting();
void DemoModeSkipping();
void DemoModeWalking();
void DemoModeRunning();
void DemoModeSwimming();
void DemoModeDissasemble();
...
int main(void)
{
 ...
 audiocontext = alcGetCurrentContext();
    ...
    if (audiodevice)
    {
        ...
        alutLoadWAVFile((ALbyte *)"run.wav", &audioalformat, &audioaldata, &audioalsize, &audioalfreq, &audioalloop);
        alBufferData(audiobuffer, audioalformat, audioaldata, audioalsize, audioalfreq);
        alutUnloadWAV(audioalformat, audioaldata, audioalsize, audioalfreq);
    ...
    if (audiodevice)
    {
        ...
        alGenBuffers(1, &audiobuffer);
        alutLoadWAVFile((ALbyte *)"swimming.wav", &audioalformat, &audioaldata, &audioalsize, &audioalfreq, &audioalloop);
        alBufferData(audiobuffer, audioalformat, audioaldata, audioalsize, audioalfreq);
        alutUnloadWAV(audioalformat, audioaldata, audioalsize, audioalfreq);
...
audiocontext = alcGetCurrentContext();
    ...
    if (audiodevice)
    {
        ...
        alGenBuffers(1, &audiobuffer);
        alutLoadWAVFile((ALbyte *)"walking.wav", &audioalformat, &audioaldata, &audioalsize, &audioalfreq, &audioalloop);
        alBufferData(audiobuffer, audioalformat, audioaldata, audioalsize, audioalfreq);
        alutUnloadWAV(audioalformat, audioaldata, audioalsize, audioalfreq);
    ...
    if (audiodevice)
    {
        ...
        alGenBuffers(1, &audiobuffer);
        alutLoadWAVFile((ALbyte *)"Sitting.wav", &audioalformat, &audioaldata, &audioalsize, &audioalfreq, &audioalloop);
        alBufferData(audiobuffer, audioalformat, audioaldata, audioalsize, audioalfreq);
        alutUnloadWAV(audioalformat, audioaldata, audioalsize, audioalfreq);
...
swimming = 0;
		...
		case 'P':
		case 'p':
			turnAngleLeftHand = turnAngleLeftHand + 1.95f;
...
GLfloat *SphereVertices = NULL;
	...
	//Basically we are giving the vertices coordinates in the above array
	Spherevertices = Sphere(0.6f, 337, &SphereVertices, &SphereTextures, &SphereNormals);
...
glGenVertexArrays(1, &vao_SpherePerFragment);
  ...
  //OpenAL 
    ...
    if (audiodevice)
    {
        ...
        alGenBuffers(1, &audiobuffer);
        alutLoadWAVFile((ALbyte *)"sound1.wav", &audioalformat, &audioaldata, &audioalsize, &audioalfreq, &audioalloop);
        alBufferData(audiobuffer, audioalformat, audioaldata, audioalsize, audioalfreq);
        alutUnloadWAV(audioalformat, audioaldata, audioalsize, audioalfreq);
	...
	// Set perspective to identity matrix
...
//	fprintf(gpLogFile, "Vlue if walking is %d\n", Walking);
	...
	else
	{
		...
		TranslateMatrix *= translate(walkingX, 2.0f, walkingTranslate);
		...
		{
		TranslateMatrix = translate(walkingX, 2.0f, walkingTranslate);
		TranslateMatrix *= rotate(turnAngleLeftHand,turnAngleRightHand, 0.0f);
		//190 for backwards
		//290.0f for right side
		modelMatrix = TranslateMatrix;
		push(modelMatrix);
		...
		if(Dissasemble == 1)
		{
			TranslateMatrix *=translate(0.0f,AD_FaceY,0.0f);
			if(AD_FaceY <= 4.8f)
			{
				AD_FaceY += 0.03f;
			}
		...
		{
			TranslateMatrix *= translate(0.0f, UpperChestSitting + 0.2f, 0.0f);
			if(Sitting == 1)
			{
				TranslateMatrix *= translate(0.0f, 0.0f, SittingFaceZ);
				if(SittingFaceZ <= 0.3f)
					SittingFaceZ += 0.006f;
			}
			else
			{
				TranslateMatrix *= translate(0.0f, 0.0f, SittingFaceZ);
				if(SittingFaceZ >= 0.0f)
					SittingFaceZ -= 0.006f;
			}
			
			TranslateMatrix *= scale(0.8f,0.8f,0.8f);
			modelMatrix = TranslateMatrix;
		}
		else if (Skipping == 1)
		{
		...
		// 	TranslateMatrix *= scale(0.9f,0.9f,0.9f);
	...
	static GLfloat RedHead;
	static GLfloat GreenHead;
	static GLfloat BlueHead;
	if(Assemble == 1)
	{
		glVertexAttrib3f(AMC_ATTRIBUTE_COLOR, RedHead,GreenHead,BlueHead);
		if(RedHead <= 1.0)
			{
				RedHead += 0.0065f;
				GreenHead += 0.0065f;
	...
	if (Sitting == 1)
	{
		...
		TranslateMatrix *=rotate(bendAngle,1.0f,0.0f,0.0f);
		if(bendAngle <10.0f)
		{
			bendAngle += 0.13f;
		}
		...
		if (UpperChestSitting > -0.5f)
			UpperChestSitting -= 0.007f;
	...
	{
		TranslateMatrix = peek();
		TranslateMatrix *= translate(0.0f, -1.6f, 0.0f);glVertexAttrib3f(AMC_ATTRIBUTE_COLOR, 1.0f, 1.0f, 1.0f);
		push(TranslateMatrix);
		if (Skipping == 1)
		{
			TranslateMatrix *= translate(0.0f, UpperChestSkippingY, 0.0f);
	...
	if(Assemble == 1)
	{
		glVertexAttrib3f(AMC_ATTRIBUTE_COLOR, r,g,b);
		if(r <= 1.0f)
		{
			r = r+0.0065f;
			g = g + 0.0065f;
	...
	else if(Dissasemble == 1)
	{
		...
		glVertexAttrib3f(AMC_ATTRIBUTE_COLOR, r,g, 0.0f);
		if(r >= 0.0f)
			{
				r -= 0.008f;
				g -= 0.008f; 
...
//	//////// Right HAND /////////////// 
	...
	else
	{
		...
		if(Dissasemble == 1)
		{
		modelMatrix *= translate(AD_RightHandX, 0.0F, 0.0f);
		if(AD_RightHandX <6.5f)
			AD_RightHandX += 0.05f;
		}
		if(Assemble == 1)
		{
		modelMatrix *= translate(AD_RightHandX, 0.0F, 0.0f);
		if(AD_RightHandX > 0.0f)
			AD_RightHandX -= 0.04f;
		}
		modelMatrix = modelMatrix * RotationMatrix;
		modelMatrix *= translate(0.0f, 0.0f, -1.0f);
		if(Sitting == 1 || Sitting == 2)
		{
			modelMatrix *= translate(0.0f, 0.0f, UpperChestSitting);
		modelMatrix *=rotate(bendAngle,1.0f,0.0f,0.0f);
		}
		push(modelMatrix);
		modelMatrix *= ScaleMatrix;
		push(modelMatrix);
...
//
	...
	static GLfloat blue;
		if(Assemble == 1)
		{
	glVertexAttrib3f(AMC_ATTRIBUTE_COLOR, r,g,blue);
			if(blue <= 1.0f)
			{
				blue +=  0.0065f;
			}
...
//Upper part Hand 
		...
		modelMatrix *= rotate((GLfloat)RightHandShoulder, 1.0f, 0.0f, 0.0f);
		modelMatrix *= translate(0.0f, 0.0f, 0.00f);
...
ScaleMatrix = scale(0.3f, 0.3f, 0.09f);
		...
		if(Dissasemble == 1)
		{
		modelMatrix *= translate(AD_LeftHandX, 0.0F, 0.0f);
		if(AD_LeftHandX > -6.5f)
			AD_LeftHandX -= 0.05f;
		}
	if(Assemble == 1)
		{
		modelMatrix *= translate(AD_LeftHandX, 0.0F, 0.0f);
		if(AD_LeftHandX < 0.0f)
			AD_LeftHandX += 0.04f;
		}
		modelMatrix = modelMatrix * RotationMatrix;
		modelMatrix *= translate(0.0f, 0.0f, -1.0f);
...
//Upper part Hand 
		...
		modelMatrix = peek();
		modelMatrix *= translate(0.0f, 0.0f, 0.8f);
		modelMatrix *= rotate((GLfloat)LeftHandShoulder, 1.0f, 0.0f, 0.0f);
		modelMatrix *= translate(0.0f, 0.0f, 0.00f);
		modelMatrix *= scale(0.3f, 0.3f, 0.3f);
 ...
 if(Sitting == 1)
	{
		...
		modelMatrix *= translate(-0.4f, 0.0f, 0.2f);
	...
	else
	{
		...
		modelMatrix *= translate(-0.4f, 0.0f, 0.18f);
		...
		if(Dissasemble == 1)
		{
				modelMatrix *= translate(AD_LeftHandX, 0.0f, AD_LegY);
				if(AD_LegY >= -4.2f)
				{
					AD_LegY -= 0.03f;
				}
	...
	static GLfloat red;
		if(Assemble == 1)
		{
	glVertexAttrib3f(AMC_ATTRIBUTE_COLOR, red,0.0f,blue);
			if(red <= 0.4f)
			{
				red +=  0.0065f;
			}
...
glVertexAttrib3f(AMC_ATTRIBUTE_COLOR,0.4f,0.0f,1.0f);
	draw_cylinder();
//
	pop();
 ...
 if(Sitting == 1 || Sitting == 2)
	{
		...
		modelMatrix = peek();
		modelMatrix *= translate(0.0f, 0.0f, 0.0f);
		modelMatrix *= rotate((GLfloat)leftLegUpper, 1.0f, 0.0f, 0.0f);
		modelMatrix *= translate(0.0f, 0.0f, 0.8f);
		modelMatrix *= scale(0.3f, 0.3f, 0.3f);
	}
	else
...
/////////// SHOES LEFT LEG
	...
	modelMatrix = peek();
		modelMatrix *= translate(0.0f, 0.0f, -0.1f);
		modelMatrix *= rotate((GLfloat)leftLegLower, 1.0f, 0.0f, 0.0f);
		modelMatrix *= translate(0.0f, -0.15f, -0.7f);
		modelMatrix *= scale(0.3f, 0.5f, 0.3f);
...
modelMatrix = peek();
		...
		if(Sitting == 1||Sitting == 2)
		modelMatrix *= translate(0.0f, 0.0f, 0.15f);
		else
		{
			modelMatrix *= translate(0.0f, 0.0f, -0.0f);
		}
...
else
{
	...
	modelMatrix *= translate(0.0f, 0.0f, -1.0f);
		if(Dissasemble == 1)
		{
				modelMatrix *= translate(AD_RightHandX, 0.0f, AD_LegY);
				if(AD_LegY >= -4.6f)
				{
					AD_LegY -= 0.03f;
				}
...
void DemoMode()
{
		...
		else if(r >= 1.0f)
		{
			...
			if(Walking == 0)
			{
				walkingTranslate = -25.0f;
				Walking = -1;
				turnAngleRightHand = 0;
				printf("I am here in WAlking ---------------------\n"); 
				//turnAngleLeftHand = 0;
			}
		}
...
void DemoModeSkipping()
{
	...
	if(Skipping == 1)
	{
		...
		if(counter >= 150 && counter <=152)
		{
			...
			if(turnAngleLeftHand <= 90.0f)
			{
				turnAngleLeftHand += 0.85f; 
			}
			if(turnAngleLeftHand >= 90.0f)
				Skipping = -1;
		}
	if(counter >= 300 && counter <= 302)
	{
		Skipping = 0;
			 shoulder = -90;
 			elbow = -30;
	...
	if(counter == 600)
	{
		...
		counter += 5;
		 ...
		 turnAngleLeftHand = 94.043999f;
		 turnAngleRightHand = 0.0f;
		 swimming = -1;
...
void DemoModeWalking()
{
	printf("Value of turnAngleLeftHAnd is %f\n",turnAngleLeftHand);
...
void DemoModeSwimming()// turnAngleLeftHand = 94.043999f;
		// turnAngleRightHand = 0.0f;
		// swimming = -1;
...
if(swimming == -1)
	{
			...
			swimming = 1;
				...
				RightLegLower = 30.0f;
 				...
 				RightHandShoulder = -90.0f;
 				RightHandElbow = 0.0f;
				walkingTranslate = -20.0f;
			//	printf("value of walking is %d\n",Walking);
	}
	if(swimming == 1)
	counter += 1;

		printf("I am in swimming and value of counter is %d\n",counter);
	if(counter == 600)
	{
		turnAngleLeftHand = 0; 
		turnAngleRightHand = 0;
...
if(Sitting == 2)
{
	...
	turnAngleLeftHand += 0.85f; 
	//Sitting = 0;
		//printf("VALUE OF Sitting IS %d\n",Sitting);
	}
	}
	if(turnAngleLeftHand == 90.099907f)
	{
	//	if(Sitting == 0)
		Sitting = 1;
		turnAngleLeftHand += 0.001;
		//	printf("VALUE OF Sitting IS %d\n",Sitting);
...
//////////THird ROtation SItting ///////

if(leftLegUpper >= -90.0f)
	{
		if(turnAngleLeftHand >= 90.0f && turnAngleLeftHand <= 180.0f)
		{
			turnAngleLeftHand += 0.85f;
		}
	
	}
		if(turnAngleLeftHand == 180.201218f)
		{
			Sitting = 1;
			turnAngleLeftHand += 0.001f;
				printf("VALUE OF Sitting IS %d\n",Sitting);
		}
if(leftLegUpper >= -90.0f)
	{
		if(turnAngleLeftHand >= 180.0f && turnAngleLeftHand <= 270.0f)
		{
			turnAngleLeftHand += 0.85f;
		}
	
	}
		if(turnAngleLeftHand == 270.302856f)
		{
			Sitting = 1;
			turnAngleLeftHand += 0.001f;
				printf("VALUE OF Sitting IS %d\n",Sitting);
		}
		if(Counter >= 4 && Counter <=100)
		{
			Counter += 1;
...
void DemoModeDissasemble()
{
		if(Dissasemble == -1)
		{
			//	printf("I am in DISSASEMBLE and value of DISSASEMVLE IS %d\n",Dissasemble);
			AD_LeftHandX = 0.0f;
			AD_RightHandX = 0.0f;
			AD_LegY = 0.0f;
			AD_FaceY = 0.0f;
	...
	void UpdateSkipping()
{
	if (Skipping == 1)
	{
		if (Jump == 1)
		{
			...
			{
				RightLegLower = RightLegLower + 0.98;
				leftLegLower = leftLegLower + 0.98;


				LeftHandElbow = LeftHandElbow - 4.35;
				RightHandElbow = RightHandElbow + 4.35;
				//fprintf(gpLogFile, "The value of leftLegLower is %f\n", leftLegLower);

				if (UpperChestSkippingY <= 0.2)
					UpperChestSkippingY += 0.001f;
...
//printf("I am here in turnAngleLeftHand %f\n",turnAngleLeftHand);
	if (Walking == 1)
	{
		//Sitting = 0;
		if(turnAngleLeftHand >= 0.0f && turnAngleLeftHand<= 90.0f)
		{
			if (walkingTranslate <= 5.0f)
				walkingTranslate += 0.08f;
		}
		else if(turnAngleLeftHand >90.0f && turnAngleLeftHand <180.0f)
		{
			//	printf("VAlue of turnAngleLeftHAnd is %f\n",turnAngleLeftHand);
			if (walkingX <= 5.0f)
				walkingX += 0.08f;
		}
		else if(turnAngleLeftHand >= 180.0f && turnAngleLeftHand <= 270.0f)
		{
			turnAngleLeftHand = 180.000000f;
		
			if (walkingTranslate >= -25.0f)
				walkingTranslate -= 0.08f;
		}
		else if(turnAngleLeftHand >=270.0f && turnAngleLeftHand <=360.0f)
		{
			if (walkingX >= -25.0f)
				walkingX -= 0.08f;
		}
		...
		if (RightLeg == 1)
		{
			if (RightLegUpper >= -141.0)
			{
				...
				LeftHandShoulder = (int)(LeftHandShoulder - 3.0) % 360;
			//	LeftHandElbow = (int)(LeftHandElbow - 2.0) % 360;

				RightHandShoulder = (int)(RightHandShoulder + 3.0) % 360;
//				RightHandElbow = (int)(RightHandElbow + 2.0) % 360;


				leftLegUpper = (int)(leftLegUpper + 3.0) % 360;
		//		leftLegLower = (int)(leftLegLower + 2.0) % 360;


				RightLegUpper = (int)(RightLegUpper - 3.0) % 360;
			//	RightLegLower = (int)(RightLegLower - 2.0) % 360;
			}
		}
	...
	if (Running == 1)
	{
		...
		if (RightLeg == 1)
		{
			if (RightHandShoulder >= -132.000076)
			{
				...
				RightHandShoulder = RightHandShoulder - 3.6f;
				RightHandElbow = RightHandElbow - 3.7f;


				LeftHandShoulder = LeftHandShoulder + 3.6f;
				LeftHandElbow = LeftHandElbow + 3.7f;

				leftLegUpper = leftLegUpper - 3.7;
				leftLegLower = leftLegLower - 3.6;


				RightLegUpper = RightLegUpper + 3.7;
				RightLegLower = RightLegLower + 3.6;
...
{
	static int counter;
//	printf("I am in swimming and the counter is %d\n",swimming);
	...
	if (swimming == 1)
	{
		...
		if(counter ==100)
		{
			turnAngleRightHand = 95.0f;
		}
		 else if(counter == 200)
		 {
		 		turnAngleRightHand =  290.0f;
		 } 
		 else if(counter == 300)
		 {
			 turnAngleRightHand = 190.0f;
		 }
		...
		if (smimmingLegsToggle == 0)
		{
			...
			{
				RightLegUpper = RightLegUpper + 1.95f;
				leftLegUpper -= 1.95f;
		...
		RightHandShoulder = RightHandShoulder - 2.45f;
		LeftHandShoulder = LeftHandShoulder + 2.45f;
	}
}
...
//
	...
	{
		glDeleteBuffers(1, &vbo_Normals_Sphere);
