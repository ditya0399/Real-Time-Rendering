...
//////////////////////////////////////For Fade In
GLfloat FadeInMix = 2.0f;
GLfloat FadeInMixFlowers = 2.0f;

GLuint FadeInMixUniformGrass;
GLuint FadeInMixUniformTrees;
GLuint FadeInMixUniformFlower;
GLuint FadeInMixUniformTerrain;

///////////////////////////////// For Fade Out
GLfloat FadeOutcount = 30.0f;
...
GLuint T_hmBMP;
GLuint T_vao;
GLuint T_vbo, T_modelUniform, T_projectionUniform, T_viewUniform;
...
GLuint N_vao;
GLuint N_vbo;
GLuint N_modelUniform;
GLuint N_viewUniform;
GLuint N_projectionUniform;
...
GLuint texture4;
GLuint texture5;
GLuint TB_gShaderProgramObject;
GLuint TB_samplerUniform;
GLuint TB_samplerUniformSandGrass;
GLuint TB_dmap_depthUniform;
GLuint TB_tex_color[5];
GLuint TB_terrainHeightMap;
GLuint TB_sandGrass;
GLuint TB_vao;
GLuint TB_vbo;
GLuint TB_modelUniform;
GLuint TB_viewUniform;
GLuint TB_projectionUniform;
GLuint TB_modelViewUniform;
GLuint TB_terrainFadeMix;
mat4 TB_perspectiveProjectionMatrix;
GLfloat TB_gWidth, TB_gHeight;
float TB_depthMapCount = 1.0f;
...
void initializeBackTerrain();
void displayBackTerrain();
...
void initializeBackTerrain()
{
	...
	const GLchar *tessellationControlShaderSourceCode =
		...
		"{" \
		"vec4 p0 = u_p_matrix * u_view_matrix * u_model_matrix * gl_in[0].gl_Position;" \
		"vec4 p1 = u_p_matrix * u_view_matrix * u_model_matrix * gl_in[1].gl_Position;" \
		"vec4 p2 = u_p_matrix * u_view_matrix * u_model_matrix * gl_in[2].gl_Position;" \
		"vec4 p3 = u_p_matrix * u_view_matrix * u_model_matrix * gl_in[3].gl_Position;" \
		"p0 /=p0.w;" \
		"p1 /=p1.w;" \
		"p2 /=p2.w;" \
		"p3 /=p3.w;" \
	...
	const GLchar *tessellationEvalutationShaderSourceCode =
		...
		"vec4 P_eye = u_view_matrix *u_model_matrix* p;" \
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"float fScale = fs_in.world_coord.y/dmap_depth * 1.4; " \
		...
		"vec4 vPathIntensity = texture2D(tex_color[4],vPathCoord);" \
		...
		"Terrain = mix(vec4(vMixedColor),vec4(0.0),FadeMix);" \
	...
	TB_gShaderProgramObject = glCreateProgram();
	//attach vertex shader to the gShaderProgramObject
	glAttachShader(TB_gShaderProgramObject, gVertexShaderObject);
	glAttachShader(TB_gShaderProgramObject, gTesselationControlShaderObject);
	glAttachShader(TB_gShaderProgramObject, gTesselationEvaluationShaderObject);
	glAttachShader(TB_gShaderProgramObject, gFragmentShaderObject);


	//Pre-Linking  binding to vertexAttributes
	glBindAttribLocation(TB_gShaderProgramObject, AMC_ATTRIBUTE_POSITION, "vPosition");
	...
	glLinkProgram(TB_gShaderProgramObject);
	...
	glGetProgramiv(TB_gShaderProgramObject, GL_LINK_STATUS, &iProgramLinkStatus);
	if (iProgramLinkStatus == GL_FALSE)
	{
		glGetProgramiv(TB_gShaderProgramObject, GL_INFO_LOG_LENGTH, &iInfoLogLength);
		if (iInfoLogLength > 0)
		{
			szInfoLog = (GLchar *)malloc(iInfoLogLength);
			if (szInfoLog != NULL)
			{
				GLsizei written3;
				glGetProgramInfoLog(TB_gShaderProgramObject, iInfoLogLength, &written3, szInfoLog);
				fprintf(gpLogFile, "%s\n", szInfoLog);
				free(szInfoLog);
				uninitialize();
				DestroyWindow(hwnd);
	...
	TB_modelUniform = glGetUniformLocation(TB_gShaderProgramObject, "u_model_matrix");
	TB_projectionUniform = glGetUniformLocation(TB_gShaderProgramObject, "u_p_matrix");
	TB_viewUniform = glGetUniformLocation(TB_gShaderProgramObject, "u_view_matrix");
	TB_dmap_depthUniform = glGetUniformLocation(TB_gShaderProgramObject, "dmap_depth");
	TB_samplerUniform = glGetUniformLocation(TB_gShaderProgramObject, "tex_displacement");
	TB_tex_color[0] = glGetUniformLocation(TB_gShaderProgramObject, "tex_color[0]");
	TB_tex_color[1] = glGetUniformLocation(TB_gShaderProgramObject, "tex_color[1]");
	TB_tex_color[2] = glGetUniformLocation(TB_gShaderProgramObject, "tex_color[2]");
	TB_tex_color[3] = glGetUniformLocation(TB_gShaderProgramObject, "tex_color[3]");
	TB_tex_color[4] = glGetUniformLocation(TB_gShaderProgramObject, "tex_color[4]");
	TB_terrainFadeMix = glGetUniformLocation(TB_gShaderProgramObject, "FadeMix");
	...
	glGenVertexArrays(1, &TB_vao);
	glBindVertexArray(TB_vao);
	...
	//glActiveTexture(GL_TEXTURE2);
	loadTexture(&texture0, MAKEINTRESOURCE(IDBITMAP_HEIGHTMAPBACK));
	//glActiveTexture(GL_TEXTURE3);

	loadTexture(&texture1, MAKEINTRESOURCE(IDBITMAP_SAND));

	//	tex_color = load("terragen_color.ktx", 0);
	//glActiveTexture(GL_TEXTURE4);
	loadTexture(&texture2, MAKEINTRESOURCE(IDBITMAP_GRASS));

	//glActiveTexture(GL_TEXTURE5);
	loadTexture(&texture3, MAKEINTRESOURCE(IDBITMAP_ROCK));

	//glActiveTexture(GL_TEXTURE6);
	loadTexture(&texture4, MAKEINTRESOURCE(IDBITMAP_SAND));

	//glActiveTexture(GL_TEXTURE7);
	loadTexture(&texture5, MAKEINTRESOURCE(IDBITMAP_PATH));
}
void initializeTerrain()
{
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"FragColor = landscape;" \
		"Terrain = mix(vec4(FragColor),vec4(0.0),FadeMix);"\
		"}";
	...
	T_modelUniform = glGetUniformLocation(T_gShaderProgramObject, "u_model_matrix");
	T_projectionUniform = glGetUniformLocation(T_gShaderProgramObject, "u_p_matrix");
	T_viewUniform = glGetUniformLocation(T_gShaderProgramObject, "u_view_matrix");
	...
	FadeInMixUniformTerrain = glGetUniformLocation(T_gShaderProgramObject, "FadeMix");
...
void initializeGrass()
{
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"{" \
			...
			"outputColor = mix(vec4(outputColor),vec4(0.0),FadeMix);" \
	...
	FadeInMixUniformGrass = glGetUniformLocation(G_gShaderProgramObject, "FadeMix");
...
void initializeNoise()
{
	...
	const GLchar *vertexShaderSourceCode =
		...
		"void main(void)" \
		"{" \
		"vec3 ECposition = vec3(u_view_matrix  *u_model_matrix * vPosition);" \
	...
	N_modelUniform = glGetUniformLocation(N_gShaderProgramObject, "u_model_matrix");
	N_viewUniform = glGetUniformLocation(N_gShaderProgramObject, "u_view_matrix");
	N_projectionUniform = glGetUniformLocation(N_gShaderProgramObject, "u_projection_matrix");
...
void initializeFlowers()
{
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"FlowerColor = mix(vec4(FragColor),vec4(0.0),FadeMix);" \
	...
	FadeInMixUniformFlower = glGetUniformLocation(Flowers_gShaderProgramObject, "FadeMix");
...
void initializeTree()
{
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"FragColor = vec4(phongADSLight,1.0);" \
		"TreeColor = mix(vec4(FragColor),vec4(0.0),FadeMix);"\
	...
	FadeInMixUniformTrees = glGetUniformLocation(gShaderProgramObject, "FadeMix");
...
void drawGeometry()
{
	perspectiveProjectionMatrix = mat4::identity();
	perspectiveProjectionMatrix = perspective(60.0f,
		(GLfloat)gWidth / (GLfloat)gHeight,
		0.1f,
		1000.0f);
...
void drawFlowers()
{
	...
	glUniform1f(FadeInMixUniformFlower, FadeInMix);
	// unuse program
...
void drawLavenderFlowers()
{
	...
	glUniformMatrix4fv(Flowers_ViewUniform, 1, GL_FALSE, viewMatrix);
	glUniformMatrix4fv(Flowers_projectionUniform, 1, GL_FALSE, perspectiveProjectionMatrix);
	glUniform1f(FadeInMixUniformFlower, FadeInMixFlowers);
...
void drawTrees()
{
	...
	glUniform1f(FadeInMixUniformTrees, FadeInMix);
...
void displayBackTerrain()
{
	glUseProgram(TB_gShaderProgramObject);
...
//	viewMatrix = lookat(vec3(xView, 5.0f, zView), vec3(xView, 2.0f, -100.0f), vec3(0.0f, 1.0f, 0.0f));
	modelMatrix = translate(0.0f, 6.0f, -95.0f);
//	modelMatrix *= rotate(270.0f, 0.0f, 1.0f,0.0f);
	...
	glUniform1i(TB_terrainHeightMap, 0);



	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, texture1);
	glUniform1i(TB_tex_color[0], 1);


	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, texture2);
	glUniform1i(TB_tex_color[1], 2);


	glActiveTexture(GL_TEXTURE3);
	glBindTexture(GL_TEXTURE_2D, texture3);
	glUniform1i(TB_tex_color[2], 3);


	glActiveTexture(GL_TEXTURE4);
	glBindTexture(GL_TEXTURE_2D, texture4);
	glUniform1i(TB_tex_color[3], 4);


	glActiveTexture(GL_TEXTURE5);
	glBindTexture(GL_TEXTURE_2D, texture5);
	glUniform1i(TB_tex_color[4], 5);
	glUniform1f(TB_terrainFadeMix, FadeInMix);
	// send necessary matrices to shader in respective uniforms
	glUniformMatrix4fv(TB_modelUniform, 1, GL_FALSE, modelMatrix);
	glUniformMatrix4fv(TB_viewUniform, 1, GL_FALSE, viewMatrix);
	glUniformMatrix4fv(TB_projectionUniform, 1, GL_FALSE, perspectiveProjectionMatrix);
	glUniform1f(TB_dmap_depthUniform, 40.5f);
	glBindVertexArray(TB_vao);
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
...
void drawterrain()
{
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, T_hmBMP);
	...
	glUniformMatrix4fv(T_modelUniform, 1, GL_FALSE, modelMatrix);
	glUniformMatrix4fv(T_viewUniform, 1, GL_FALSE, viewMatrix);
	glUniformMatrix4fv(T_projectionUniform, 1, GL_FALSE, perspectiveProjectionMatrix);
	glUniform1f(T_dmap_depthUniform, 10.5f);
	glUniform1f(FadeInMixUniformTerrain, FadeInMix);
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
...
void drawGrass()
{
	...
	glUniform1f(FadeInMixUniformGrass, FadeInMix);
...
void drawNoise()
{
	...
	if (Noise == 1)
	{
		...
		glUniformMatrix4fv(N_modelUniform, 1, GL_FALSE, modelMatrix);
		glUniformMatrix4fv(N_viewUniform, 1, GL_FALSE, viewMatrix);
		glUniformMatrix4fv(N_projectionUniform, 1, GL_FALSE, perspectiveProjectionMatrix);
...
void display(void)
{
	...
	if (FadeInMixFlowers >= 0.0)
		FadeInMixFlowers -= 0.005;

	FadeOutcount = FadeOutcount - 0.05f;
	...
	if (FadeOutcount <= 0.0f)
	{
		...
		if (FadeInMixFlowers <= 2.0)
			FadeInMixFlowers += 0.0055;


		fprintf(gpLogFile, "I am in FadeOut Count\n");
		rainSpeed += 0.188f;
		rainSpeed1 += 0.255f;
		rainSpeed2 += 0.098f;
		rainSpeed3 += 0.23f;
