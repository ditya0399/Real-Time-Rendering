...
//Sphere
GLfloat XAxisRotate = 0.0f;
GLfloat YAxisRotate = 0.0f;
GLfloat ZAxisRotate = 0.0f;
...
int WindowsXInitial = 200;
int WindowsYInitial = 670;
...
GLfloat lightAmbient[4] = { 0.0,0.0,0.0,1.0f };
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR szCmdLine, int iCmdShow)
{
	...
	fprintf(gpLogFile, "I am in WinMain\n");
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("24SpheresWithOneLightAndDifferentMaterials"),
...
{
	void resize(int, int);
//	fprintf(gpLogFile, "I am in WndProc\n");
	...
	case WM_SIZE:
		fprintf(gpLogFile, "I am in WM_SIZE\n");
	...
	case WM_CHAR:
		...
		{
		case VK_ESCAPE:
			fprintf(gpLogFile, "I am in CASE ESCAPE\n");
			ToggleFullScreen();
			DestroyWindow(hwnd);
			break;
...
int initialize(void)
{
	//fprintf(gpLogFile, "I am in Initialize\n");
...
{
	//fprintf(gpLogFile, "I am in Resize\n");
	//gpLogFile, "WIDTH : %d\n HEIGHT: %d\n", width, height);
...
void display(void)
{
//	fprintf(gpLogFile, "I am in Display\n");
	//fprintf(gpLogFile, "I am in Display : WIDTH : %d\n  HEIGHT : %d\n",Win_Width,Win_Height);
	...
	for (int i = 0; i <= 3; i++)
	{
		...
		if (gbLighting == 1)
		{
			...
			glUniform3fv(KDUniform, 1, materials[i].materialDiffuse);
			glUniform3fv(KAUniform, 1, materials[i].materialAmbient);
			glUniform3fv(KSUniform, 1, materials[i].materialSpecular);
			glUniform1f(MaterialShininessUniform, materials[i].materialShininess);
			if (x == 1)
			{
				
				lightPosition[0] = 0.0f;
				lightPosition[1] = sin(XAxisRotate) * 100.0f;
				lightPosition[2] = cos(XAxisRotate) * 100.0f;

			}
			if (y == 1)
			{
				
				lightPosition[0] = sin(YAxisRotate) * 100.0f;
				lightPosition[1] = 0.0f;
				lightPosition[2] = cos(YAxisRotate) * 100.0f;

			}
			if (z == 1)
			{
			
				lightPosition[0] = sin(ZAxisRotate) * 100.0f;
				lightPosition[1] = cos(ZAxisRotate) * 100.0f;
		...
		WindowsHeight = 720;
		WindowsYInitial = 650;
		fprintf(gpLogFile, "I am in 1st Loop :\n");
		glUseProgram(gShaderProgramObject);
		drawSphere();
		//200, 670, 300, 720
		if (WindowsXInitial < 1200 && WindowsWidth < 1300)
		{
			WindowsXInitial += 250;
			WindowsWidth += 250;
		}
	...
	for (int j = 4; j <= 7; j++)
	{
		if (gbLighting == 1)
		{
			...
			glUniform3fv(KDUniform, 1, materials[j].materialDiffuse);
			glUniform3fv(KAUniform, 1, materials[j].materialAmbient);
			glUniform3fv(KSUniform, 1, materials[j].materialSpecular);
			glUniform1f(MaterialShininessUniform, materials[j].materialShininess);
			glUniform4fv(LightPositionUniform, 1, lightPosition);
		}
		else
		{
		...
		if (WindowsYInitial > 465)
		{
			//	fprintf(gpLogFile, "I am out of loop\n");
				//WindowsHeight -= 100;
			WindowsYInitial -= 185;
		}
		
		fprintf(gpLogFile, "I am in 2nd Loop :\n");
		drawSphere();
		//200, 670, 300, 720
		if (WindowsXInitial < 1200 && WindowsWidth < 1300)
		{
	...
	glUseProgram(0);
	/*if (WindowsHeight >= 420 && WindowsYInitial >= 370)
	{
		fprintf(gpLogFile, "I am out of loop\n");
		WindowsHeight -= 100;
		WindowsYInitial -= 100;
...
void drawSphere(void)
{
	...
	//200, 670, 1080, 720
	glViewport(WindowsXInitial, WindowsYInitial, WindowsWidth, WindowsHeight);
	perspectiveProjectionMatrix = perspective(45.0f,
		(GLfloat)WindowsWidth / (GLfloat)WindowsHeight,
		0.1f,
		100.0f);
...
void uninitialize(void)
{
	//fprintf(gpLogFile, "I am in UnInitialize\n");
...
{
	fprintf(gpLogFile, "I am in ToggleFullScreen\n");
	MONITORINFO MI;
