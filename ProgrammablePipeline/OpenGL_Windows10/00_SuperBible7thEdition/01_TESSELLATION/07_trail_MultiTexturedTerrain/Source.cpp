...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case VK_UP:
			depthMapCount += 1.0f;
			fprintf(gpLogFile, "Value of depthMap is %f\n", depthMapCount);
			break;
...
int initialize(void)
{
	...
	loadTexture(&texture0, MAKEINTRESOURCE(IDBITMAP_SMILEY));
//	glActiveTexture(GL_TEXTURE1);
	...
	//glActiveTexture(GL_TEXTURE2);
	loadTexture(&texture2, MAKEINTRESOURCE(IDBITMAP_HEIGHTMAP));
...
//	modelViewMatrix = vmath::lookat(vec3(0.0f, 35.0f, 85.0f), vec3(0.0f, 0.0f, -255.0f), vec3(0.0f, 1.0f, 0.0f));
	...
	glUniform1i(terrainHeightMap, 2);



	glActiveTexture(GL_TEXTURE3);
	glBindTexture(GL_TEXTURE_2D, texture1);
	glUniform1i(tex_color[0], 3);


	glActiveTexture(GL_TEXTURE4);
	glBindTexture(GL_TEXTURE_2D, texture2);
	glUniform1i(tex_color[1], 4);


	glActiveTexture(GL_TEXTURE5);
	glBindTexture(GL_TEXTURE_2D, texture3);
	glUniform1i(tex_color[2], 5);


	glActiveTexture(GL_TEXTURE6);
	glBindTexture(GL_TEXTURE_2D, texture4);
	glUniform1i(tex_color[3], 6);


	glActiveTexture(GL_TEXTURE7);
	glBindTexture(GL_TEXTURE_2D, texture5);
	glUniform1i(tex_color[4], 7);
	...
	glUniform1f(dmap_depthUniform, depthMapCount);
