...
void Noise()
{
	...
	for (k = 0; k < SIZE; k++)
	{
		noise1[i][j][k] = (float)((rand() % 32768) / 32768.0);
...
{
	float t1;
	float p_l, p_l2, p_r, p_r2, p_face1, p_face2, p_final;
	...
	py = (int)pnt.y;
	pz = (int)pnt.z;
	x = px & (SIZE);//make sure the values are in table
	y = py & (SIZE);
	z = pz & (SIZE);


	t1 = pnt.y - py;
	p_l = calcnoise[x][y][z + 1] + t1 * (calcnoise[x][y + 1][z + 1] - 
		calcnoise[x][y][z + 1]);
	p_r = calcnoise[x + 1][y][z + 1] + t1 * (calcnoise[x + 1][y + 1][z + 1] - 
		calcnoise[x + 1][y][z + 1]);
	p_l2 = calcnoise[x][y][z] + t1 * (calcnoise[x][y + 1][z] - 
		calcnoise[x][y][z]);
	p_r2 = calcnoise[x + 1][y][z] + t1 * (calcnoise[x + 1][y + 1][z] - calcnoise[x + 1][y][z]);
