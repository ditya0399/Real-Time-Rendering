...
int initialize(void)
{
	...
	modelViewProjectionUniform = glGetUniformLocation(gShaderProgramObject, "u_modelViewProjectionMatrix");
...
void display(void)
{
	...
	modelViewMatrix = modelViewMatrix * lookat(CameraPos, CameraFocus, CameraUp);
	...
	//modelViewProjectionMatrix = peek();
	...
	modelViewMatrix *= scale(0.20f, 0.20f, 0.20f);
	push(modelViewMatrix);
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	modelViewProjectionMatrix = perspectiveProjectionMatrix * peek();
	glUniformMatrix4fv(modelViewProjectionUniform, 1, GL_FALSE, modelViewProjectionMatrix);
