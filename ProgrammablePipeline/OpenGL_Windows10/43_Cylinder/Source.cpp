...
int initialize(void)
{
	...
	vertices = Cylinder(&Vertices, 0.9f, 1.0f);
...
void display(void)
{
	...
	RotationMatrix = rotate(90.0f, 0.0f, 0.0f);
	// perform necessary transformations
	modelMatrix = translate((float)(sin(RedLightRotate)), 1.5f, (float)(cos(RedLightRotate)));
	// do necessary matrix multiplication
	// this was internally done by glOrtho() in FFP.	


	viewMatrix = lookat(vec3(0.0f, 0.0f, 6.0f), vec3(0.0f, 0.0, -10.0f), vec3(0.0f, 1.0f, 0.0f));
	...
	glBindVertexArray(vao);
	glDrawArrays(GL_POINTS, 0, 3 * vertices);
	// unbind vao
	glBindVertexArray(0);
