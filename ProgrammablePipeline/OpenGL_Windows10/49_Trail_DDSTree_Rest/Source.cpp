...
///////// Variables for BillBoarded trees

vector< vector< glm::vec3> > TR_vVertexData;

glm::vec3 TR_vRenderScale;
int TR_iNumGrassTriangles;
bool TR_bLoaded;
int TR_iRows;
int TR_iCols;
int TR_i;
...
GLuint TR_uiBuffer;
GLuint TR_uiGrassVAO;


GLuint TR_viewUniform;
GLuint TR_vEyeUniform;
GLuint TR_modelUniform;
GLuint TR_vColorUniform;
GLuint TR_fAlphaTest;
GLuint TR_fAlphaMultiplier;
GLuint TR_NormalMatrixUniform;
GLuint TR_projectionUniform;
GLuint TR_gSampler;


float TR_fAlphaTestVAR = 0.25f;
float TR_fAlphaMultiplierVAR = 1.5f;


vector<BYTE> TR_data1;
int TR_iCurrentSize;



GLuint TR_uiTexture;
int TR_iWidth, TR_iHeight, TR_iBPP;
string TR_sPath;
bool TR_bMipMapsGenerated;
GLuint TR_uiSampler;
int TR_tfMinification, TR_tfMagnification;
GLuint TR_fTimePassedUniform;
float TR_time;

//Variables for textures
GLuint TR_vbo_texture;
GLuint TR_smileyTexture;
GLuint TR_samplerUniform;

GLuint TR_vao;
GLuint TR_vbo;
GLuint TR_mvpUniform;
GLuint TR_gShaderProgramObject;
...
bool LoadHeightMapFromImageFreeTrees(string);
void AddData(void* ptrData, UINT uiDataSize);
void AddDataTrees(void* ptrData, UINT uiDataSize);
void LoadAllTextures();
void LoadAllTexturesTrees();
...
float GetHeightFromRealVectorTrees(glm::vec3 vRealPosition);
void initializeGrass();
void initializeTrees();
bool LoadTexture2DTrees(string a_sPath, bool bGenerateMipMaps);
void SetFilteringTrees(int a_tfMagnification, int a_tfMinification);
void SetWrapTrees();
void SetAnisotropyLevelTrees(float fAnisotropyLevel);
void CreateFromDataTrees(BYTE* bData, int a_iWidth, int a_iHeight, int a_iBPP, GLenum format, bool bGenerateMipMaps);
...
void initializeTrees()
{
	...
	//attach vertex shader to the gShaderProgramObject
	glAttachShader(TR_gShaderProgramObject, gVertexShaderObject);
	glAttachShader(TR_gShaderProgramObject, gGeometryShaderObject);
	//attach fragment shader to the gShaderProgramObject
	glAttachShader(TR_gShaderProgramObject, gFragmentShaderObject);
	...
	//Link the shader program 
	glLinkProgram(TR_gShaderProgramObject);
	...
	glGetProgramiv(TR_gShaderProgramObject, GL_LINK_STATUS, &iProgramLinkStatus);
	if (iProgramLinkStatus == GL_FALSE)
	{
		glGetProgramiv(TR_gShaderProgramObject, GL_INFO_LOG_LENGTH, &iInfoLogLength);
		if (iInfoLogLength > 0)
		{
			szInfoLog = (GLchar *)malloc(iInfoLogLength);
			if (szInfoLog != NULL)
			{
				GLsizei written3;
				glGetProgramInfoLog(TR_gShaderProgramObject, iInfoLogLength, &written3, szInfoLog);
				fprintf(gpLogFile, "%s\n", szInfoLog);
				free(szInfoLog);
				uninitialize();
				DestroyWindow(hwnd);
	...
	//POST Linking
	//Retrieving uniform locations 
	TR_modelUniform = glGetUniformLocation(TR_gShaderProgramObject, "matrices.modelMatrix");
	TR_viewUniform = glGetUniformLocation(TR_gShaderProgramObject, "matrices.viewMatrix");
	TR_vEyeUniform = glGetUniformLocation(TR_gShaderProgramObject, "vEyePosition");
	TR_projectionUniform = glGetUniformLocation(TR_gShaderProgramObject, "matrices.projMatrix");
	TR_NormalMatrixUniform = glGetUniformLocation(TR_gShaderProgramObject, "matrices.normalMatrix");
	TR_vColorUniform = glGetUniformLocation(TR_gShaderProgramObject, "vColor");
	TR_fTimePassedUniform = glGetUniformLocation(TR_gShaderProgramObject, "fTimePassed");
	TR_fAlphaTest = glGetUniformLocation(TR_gShaderProgramObject, "fAlphaTest");
	TR_fAlphaMultiplier = glGetUniformLocation(TR_gShaderProgramObject, "fAlphaMultiplier");
	TR_gSampler = glGetUniformLocation(TR_gShaderProgramObject, "gSampler");

	//Here we have done all the preparations of data transfer from CPU to GPU
	LoadAllTexturesTrees();
	const GLfloat triangleVertices[] =
	{
		0.0f,1.0f,0.0f,
		-1.0f,-1.0f,0.0f,
	...
	LoadHeightMapFromImageFreeTrees("heightmap.bmp");
...
//	modelViewProjectionMatrix = mat4::identity();
//	modelViewMatrix = mat4::identity();
//	modelMatrix = translate(10.0f, 12.0f, 25.0f);
...
//	glUniform4f(N_CloudColor, 0.8f, 0.8f, 0.8f, 0.5f);
...
//
	...
	//glActiveTexture(GL_TEXTURE11);
	//glUniform1i(TR_gSampler, 11);
	glUniformMatrix4fv(TR_projectionUniform, 1, GL_FALSE, (GLfloat *)&perspectiveProjectionMatrix);
	glUniformMatrix4fv(TR_viewUniform, 1, GL_FALSE, (GLfloat *)&viewMatrix);
	//glUniformMatrix4fv(G_modelUniform, 1, GL_FALSE, (GLfloat *)&(glm::mat4(1.0)));
	glUniformMatrix4fv(TR_modelUniform, 1, GL_FALSE, modelMatrix);
	glUniform1f(TR_fTimePassedUniform, time);
	time += 0.002f;
	glBindVertexArray(TR_uiGrassVAO);
	glDrawArrays(GL_POINTS, 0, TR_iNumGrassTriangles);
	//glEnable(GL_DEPTH_TEST);
	glBindVertexArray(0);
...
{
	vRenderScale = glm::vec3(200.0f, 25.0f, 20.0f);
...
void LoadAllTextures()
{
	...
	LoadTexture2D("data\\textures\\grassPack5.dds", true);
...
void LoadAllTexturesTrees()
{
	...
	LoadTexture2DTrees("tree1.dds", true);
	SetFilteringTrees(TEXTURE_FILTER_MAG_BILINEAR, TEXTURE_FILTER_MIN_TRILINEAR);
	SetWrapTrees();
	SetAnisotropyLevelTrees(4.0f);
}
bool LoadTexture2D(string a_sPath, bool bGenerateMipMaps)
{
	FREE_IMAGE_FORMAT fif = FIF_UNKNOWN;
...
bool LoadTexture2DTrees(string a_sPath, bool bGenerateMipMaps)
{
	...
	if (format != 0)
		...
		CreateFromDataTrees(bDataPointer, FreeImage_GetWidth(dib), FreeImage_GetHeight(dib), FreeImage_GetBPP(dib), format, bGenerateMipMaps);
...
void SetFilteringTrees(int a_tfMagnification, int a_tfMinification)
{
	glBindSampler(0, TR_uiSampler);

	// Set magnification filter
	if (a_tfMagnification == TEXTURE_FILTER_MAG_NEAREST)
		glSamplerParameteri(TR_uiSampler, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	else if (a_tfMagnification == TEXTURE_FILTER_MAG_BILINEAR)
		glSamplerParameteri(TR_uiSampler, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

	// Set minification filter
	if (a_tfMinification == TEXTURE_FILTER_MIN_NEAREST)
		glSamplerParameteri(TR_uiSampler, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	else if (a_tfMinification == TEXTURE_FILTER_MIN_BILINEAR)
		glSamplerParameteri(TR_uiSampler, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	else if (a_tfMinification == TEXTURE_FILTER_MIN_NEAREST_MIPMAP)
		glSamplerParameteri(TR_uiSampler, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_NEAREST);
	else if (a_tfMinification == TEXTURE_FILTER_MIN_BILINEAR_MIPMAP)
		glSamplerParameteri(TR_uiSampler, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_NEAREST);
	else if (a_tfMinification == TEXTURE_FILTER_MIN_TRILINEAR)
		glSamplerParameteri(TR_uiSampler, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);

	TR_tfMinification = a_tfMinification;
	TR_tfMagnification = a_tfMagnification;
}
void SetWrapTrees()
{
	glBindSampler(0, TR_uiSampler);
	glSamplerParameteri(TR_uiSampler, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glSamplerParameteri(TR_uiSampler, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
}
void SetAnisotropyLevelTrees(float fAnisotropyLevel)
{
	glSamplerParameterf(TR_uiSampler, GL_TEXTURE_MAX_ANISOTROPY_EXT, fAnisotropyLevel);
}
void CreateFromDataTrees(BYTE* bData, int a_iWidth, int a_iHeight, int a_iBPP, GLenum format, bool bGenerateMipMaps)
{
	glGenTextures(1, &TR_uiTexture);
	glBindTexture(GL_TEXTURE_2D, TR_uiTexture);
	if (format == GL_RGBA || format == GL_BGRA)
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, a_iWidth, a_iHeight, 0, format, GL_UNSIGNED_BYTE, bData);
	// We must handle this because of internal format parameter
	else if (format == GL_RGB || format == GL_BGR)
		...
		glTexImage2D(GL_TEXTURE_2D, 0, format, a_iWidth, a_iHeight, 0, format, GL_UNSIGNED_BYTE, bData);
	if (bGenerateMipMaps)glGenerateMipmap(GL_TEXTURE_2D);
	glGenSamplers(1, &TR_uiSampler);

	TR_sPath = "";
	TR_bMipMapsGenerated = bGenerateMipMaps;
	TR_iWidth = a_iWidth;
	TR_iHeight = a_iHeight;
	TR_iBPP = a_iBPP;
}
bool LoadHeightMapFromImageFreeTrees(string sImagePath)
{
	TR_vRenderScale = glm::vec3(200.0f, 25.0f, 20.0f);
	...
	BYTE* bDataPointer = FreeImage_GetBits(dib); // Retrieve the image data
	TR_iRows = FreeImage_GetHeight(dib);
	TR_iCols = FreeImage_GetWidth(dib);

	fprintf_s(gpLogFile, "iRows : %d\n", TR_iRows);
	fprintf_s(gpLogFile, "iCols : %d\n", TR_iCols);
	// We also require our image to be either 24-bit (classic RGB) or 8-bit (luminance)
	if (bDataPointer == NULL || TR_iRows == 0 || TR_iCols == 0 || (FreeImage_GetBPP(dib) != 24 && FreeImage_GetBPP(dib) != 8))
		return false;

	// How much to increase data pointer to get to next pixel data
	unsigned int ptr_inc = FreeImage_GetBPP(dib) == 24 ? 3 : 1;
	// Length of one row in data
	unsigned int row_step = ptr_inc * TR_iCols;

	//vboHeightmapData.CreateVBO();
	// All vertex data are here (there are iRows*iCols vertices in this heightmap), we will get to normals later
	TR_vVertexData = vector< vector< glm::vec3> >(TR_iRows, vector<glm::vec3>(TR_iCols));
	vector< vector< glm::vec2> > TR_vCoordsData(TR_iRows, vector<glm::vec2>(TR_iCols));

	float fTextureU = float(TR_iCols)*0.1f;
	float fTextureV = float(TR_iRows)*0.1f;
	int m = 0;
	FOR(i, TR_iRows)
	{
		FOR(j, TR_iCols)
		{
			float fScaleC = float(j) / float(TR_iCols - 1);
			float fScaleR = float(i) / float(TR_iRows - 1);
			float fVertexHeight = float(*(bDataPointer + row_step * i + j * ptr_inc)) / 255.0f;
			TR_vVertexData[i][j] = glm::vec3(-0.5f + fScaleC, fVertexHeight, -0.5f + fScaleR);
			TR_vCoordsData[i][j] = glm::vec2(fTextureU*fScaleC, fTextureV*fScaleR);
			//fprintf_s(gpLogFile, "vVertexData : %f \t %f\n", vVertexData[i][j]);
		//	fprintf(gpLogFile, "I am in loop %d", m);
			//m++;
		}
	...
	// Normals are here - the heightmap contains ( (iRows-1)*(iCols-1) quads, each one containing 2 triangles, therefore array of we have 3D array)
	vector< vector<glm::vec3> > TR_vNormals[2];
	FOR(i, 2)TR_vNormals[i] = vector< vector<glm::vec3> >(TR_iRows - 1, vector<glm::vec3>(TR_iCols - 1));

	FOR(i, TR_iRows - 1)
	{
		FOR(j, TR_iCols - 1)
		{
			glm::vec3 TR_vTriangle0[] =
			{
				TR_vVertexData[i][j],
				TR_vVertexData[i + 1][j],
				TR_vVertexData[i + 1][j + 1]
			};
			glm::vec3 TR_vTriangle1[] =
			{
				TR_vVertexData[i + 1][j + 1],
				TR_vVertexData[i][j + 1],
				TR_vVertexData[i][j]
			};

			glm::vec3 TR_vTriangleNorm0 = glm::cross(TR_vTriangle0[0] - TR_vTriangle0[1], TR_vTriangle0[1] - TR_vTriangle0[2]);
			glm::vec3 TR_vTriangleNorm1 = glm::cross(TR_vTriangle1[0] - TR_vTriangle1[1], TR_vTriangle1[1] - TR_vTriangle1[2]);

			TR_vNormals[0][i][j] = glm::normalize(TR_vTriangleNorm0);
			TR_vNormals[1][i][j] = glm::normalize(TR_vTriangleNorm1);

		}
	}

	vector< vector<glm::vec3> > TR_vFinalNormals = vector< vector<glm::vec3> >(TR_iRows, vector<glm::vec3>(TR_iCols));
		...
		FOR(j, TR_iCols)
	{
		...
		glm::vec3 TR_vFinalNormal = glm::vec3(0.0f, 0.0f, 0.0f);

		// Look for upper-left triangles
		if (j != 0 && i != 0)
			FOR(k, 2)TR_vFinalNormal += TR_vNormals[k][i - 1][j - 1];
		// Look for upper-right triangles
		if (i != 0 && j != TR_iCols - 1)TR_vFinalNormal += TR_vNormals[0][i - 1][j];
		// Look for bottom-right triangles
		if (i != TR_iRows - 1 && j != TR_iCols - 1)
			FOR(k, 2)TR_vFinalNormal += TR_vNormals[k][i][j];
		// Look for bottom-left triangles
		if (i != TR_iRows - 1 && j != 0)
			TR_vFinalNormal += TR_vNormals[1][i][j - 1];
		TR_vFinalNormal = glm::normalize(TR_vFinalNormal);

		TR_vFinalNormals[i][j] = TR_vFinalNormal; // Store final normal of j-th vertex in i-th row
	...
	float fGrassPatchOffsetMin = 1.5f;
	float fGrassPatchOffsetMax = 0.5f;
	float fGrassPatchHeight = 5.0f;

	glm::vec3 vCurPatchPos(-TR_vRenderScale.x*0.5f + fGrassPatchOffsetMin, 0.0f, TR_vRenderScale.z*0.5f - fGrassPatchOffsetMin);
	TR_iNumGrassTriangles = 0;

	while (vCurPatchPos.x < TR_vRenderScale.x*0.5f)
	{
		vCurPatchPos.z = TR_vRenderScale.z*0.5f - fGrassPatchOffsetMin;

		while (vCurPatchPos.z > -TR_vRenderScale.z*0.5f)
		{
			vCurPatchPos.y = GetHeightFromRealVectorTrees(vCurPatchPos) - 0.3f;
			AddDataTrees(&vCurPatchPos, sizeof(glm::vec3));

			TR_iNumGrassTriangles += 1;
	...
	glGenVertexArrays(1, &TR_uiGrassVAO);
	glBindVertexArray(TR_uiGrassVAO);
	glGenBuffers(1, &TR_uiBuffer);
	glBindBuffer(GL_ARRAY_BUFFER, TR_uiBuffer);
	glBufferData(GL_ARRAY_BUFFER, TR_data1.size(), &TR_data1[0], GL_STATIC_DRAW);
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(glm::vec3), 0);

	TR_bLoaded = true; // If get here, we succeeded with generating heightmap
	return true;
}
void AddDataTrees(void* ptrData, UINT uiDataSize)
{
	TR_data1.insert(TR_data1.end(), (BYTE*)ptrData, (BYTE*)ptrData + uiDataSize);
	TR_iCurrentSize += uiDataSize;
}
float GetHeightFromRealVectorTrees(glm::vec3 vRealPosition)
{
	int iColumn = int((vRealPosition.x + TR_vRenderScale.x*0.5f)*float(TR_iCols) / (TR_vRenderScale.x));
	int iRow = int((vRealPosition.z + TR_vRenderScale.z*0.5f)*float(TR_iRows) / (TR_vRenderScale.z));

	iColumn = glm::min(iColumn, TR_iCols - 1);
	iRow = glm::min(iRow, TR_iRows - 1);

	iColumn = glm::max(iColumn, 0);
	iRow = glm::max(iRow, 0);

	return TR_vVertexData[iRow][iColumn].y*TR_vRenderScale.y;
}
