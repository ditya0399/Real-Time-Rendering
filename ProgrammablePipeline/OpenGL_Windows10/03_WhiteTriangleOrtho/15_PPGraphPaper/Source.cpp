...
int GenerateGraphVertices(int ,GLfloat **,GLfloat **);
...
int initialize(void)
{
	...
	GLfloat *GraphVertices = NULL;
	GLfloat *GraphHorizontalVertices = NULL;
	int num;
	
	vert  = GenerateGraphVertices(5, &GraphVertices,&GraphHorizontalVertices);
	...
	glBufferData(GL_ARRAY_BUFFER, 2 * vert * sizeof(GraphVertices), GraphVertices, GL_STATIC_DRAW);
	...
	glBufferData(GL_ARRAY_BUFFER, 2 *  vert * sizeof(GLfloat), GraphHorizontalVertices, GL_STATIC_DRAW);
	glVertexAttribPointer(AMC_ATTRIBUTE_POSITION, 2, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(AMC_ATTRIBUTE_POSITION);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	glBindVertexArray(0);
...
// intialize above matrices to identity
	...
	//glLineWidth(6.0f);
	glLineWidth(1.0f);
	glDrawArrays(GL_LINES , 0, vert  * 2);
...
int GenerateGraphVertices(int Vertices, GLfloat **Vert,GLfloat **HorizontalVertices)
{
	int n,j,k;
	n = 0;
	j = 1;
	k = 3;
	int l = 0;
	*Vert = (GLfloat *)malloc(2 * sizeof(GLfloat) * 80);
	*HorizontalVertices = (GLfloat *)malloc(2 * sizeof(GLfloat) * 80);
	GLfloat *ArrayOfVertices = *Vert;
	GLfloat *HVertices = *HorizontalVertices;
	for(GLfloat iFloat = -1.0f; iFloat <= 1.0f; iFloat = iFloat + 0.1f)
 	{
	//	vert[0] = iFloat;
		ArrayOfVertices[n + 0] = iFloat;
		fprintf(gpLogFile, "The value of %d iFloat vertices are %d\n",n,ArrayOfVertices[n]);
		n = n + 2;
		ArrayOfVertices[n + 0] = iFloat;
		ArrayOfVertices[j] = 1.0f;
		fprintf(gpLogFile, "The value of j %d and vertices are %d\n",j, ArrayOfVertices[j]);
		j = j + 4;
		ArrayOfVertices[k] = -1.0f;
