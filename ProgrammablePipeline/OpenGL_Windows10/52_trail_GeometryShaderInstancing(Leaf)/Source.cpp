...
//
	...
	const GLchar *geometryShaderSource =
		...
		"float randX = 13 *rand(vec2(gl_InvocationID * 4.3,0.0));" \
		"float randY =  15.0 * rand(vec2(gl_InvocationID,0.0));" \
		"float randZ = 15 *  rand(vec2(gl_InvocationID * 19.3,0.0)) - 50.0;" \
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"if(FragColor.r +FragColor.g + FragColor.b >= 2.0 )"\
		"discard;"\
		"}" \
...
void display(void)
{
	...
	static GLfloat translateLeaves;
...
//	////////  RECTANGLE //////////////
	...
	TranslateMatrix = translate(translateLeaves, -2.0f, -50.0f);
	scaleMatrix = scale(0.009f, 0.009f, 0.009f);
	//TranslateMatrix *= scaleMatrix;

	//RotationMatrix = rotate(gAngle, 1.0f, 0.0f, 0.0f);
	//TranslateMatrix *= RotationMatrix; 

	//RotationMatrix = rotate(angle1, 0.0f, 1.0f, 0.0f);
//	RotationMatrix = rotate(angle1, 1.0f, 0.0f, 0.0f);
	/*RotationMatrix *= rotate(angle1, 0.0f, 0.0f, 1.0f);*/
	RotationMatrix = rotate(angle1, 0.0f, 1.0f, 0.0f);
	TranslateMatrix *= RotationMatrix;
	viewMatrix = lookat(vec3(xView, 0.0f, zView), vec3(xView, 0.0f, -100.0f), vec3(0.0f, 1.0f, 0.0f));
//	
...
///////////// 2ND SLOT ///////////////////////
	...
	RotationMatrix = mat4::identity();
	TranslateMatrix = mat4::identity();
	TranslateMatrix = translate(translateLeaves, -16.0f, -50.0f);
	scaleMatrix = scale(0.009f, 0.009f, 0.009f);
...
modelMatrix = translate(xView + 1.2f, 0.0f, zView - 1.0f);
//	modelMatrix = translate(0.0f, 0.0f, -50.0f);
	...
	glUniform4f(N_CloudColor, 0.8f, 0.8f, 0.8f, 0.4f);
