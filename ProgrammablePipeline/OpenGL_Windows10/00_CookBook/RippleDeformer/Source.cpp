...
"		gl_Position = MVP * vec4(vVertex.x, y, vVertex.z, 1);         " \
	...
	mvpUniform = glGetUniformLocation(gShaderProgramObject, "MVP");
	TIME = glGetUniformLocation(gShaderProgramObject, "time");
	...
	for (j = 0; j <= NUM_Z; j++) {
		for (i = 0; i <= NUM_X; i++) {
			vertices[count++] = vmath::vec3(((float(i) / (NUM_X - 1)) * 2 - 1)* HALF_SIZE_X, 0, ((float(j) / (NUM_Z - 1)) * 2 - 1)*HALF_SIZE_Z);
		}
	}
...
void display(void)
{
	...
	modelViewMatrix = translate(0.0f, -2.0f, -8.0f);
