...
int initialize(void)
{
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"else" \
		"{" \
		"phongADSLight = vec3(0.2,0.2,0.8);" \
		"}" \
	...
	b0Uniform = glGetUniformLocation(gShaderProgramObject, "b2Constant");
	b1Uniform = glGetUniformLocation(gShaderProgramObject, "b1Constant");
	...
	const GLchar *fragmentShaderSourceCodeGodRays =
		...
		"{" \
			...
			"vec2 Direction = vec2(0.8) * TexCoord.xy;" \
...
//	RotationMatrix = rotate(angleRectangle, angleRectangle, 0.0f);
	...
	lightning_vertices[0] = -0.5f;
	lightning_vertices[1] = 1.0f;
	lightning_vertices[2] = 0.5f;
	lightning_vertices[3] = -1.0f;
	lightning_vertices[4] = 1.0f;
	lightning_vertices[5] = 1.0f;
	lightning_vertices[6] = 2.0f;
	lightning_vertices[7] = -1.0f;
	glBindBuffer(GL_ARRAY_BUFFER, vbo_position_Sphere);
	glBufferData(GL_ARRAY_BUFFER, sizeof(lightning_vertices), lightning_vertices, GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER,0);
	//glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_Element_Sphere);
...
//	gbLighting = 1;
	...
	//RotationMatrix = mat4::identity();
	//TranslateMatrix = mat4::identity();
	//TranslateMatrix = translate(-1.0f, 0.0f, -8.0f);
	//modelMatrix = TranslateMatrix * RotationMatrix;
