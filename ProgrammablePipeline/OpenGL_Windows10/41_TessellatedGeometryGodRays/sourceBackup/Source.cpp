...
int initialize(void)
{
	...
	const GLchar *vertexShaderSourceCode =
		...
		"NormalMatrix = inverse(mat3(transpose(u_viewMatrix * u_modelMatrix)));" \
		"tNorm = mat3(NormalMatrix) * vNormal;" \
	...
	const GLchar *fragmentShaderSourceCodeGodRays =
		...
		"{" \
			...
			"{" \
				"Color = Color + texture2D(ColorBuffer,TexCoord).rgb * Intensity;" \
				"Intensity = Intensity * Decay;" \
				"TexCoord = TexCoord  + Direction;" \
			"}" \
