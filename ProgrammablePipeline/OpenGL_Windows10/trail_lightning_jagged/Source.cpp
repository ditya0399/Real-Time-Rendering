...
float printRandoms2(int count)
{
	int i = 0;
	float a = 6.0f;
	
		float num = (float)(rand() / (float)(RAND_MAX))* 130.0 - 50.0;
		//printf("%f\n", num);
...
int initialize(void)
{
	...
	const GLchar *vertexShaderSourceCode =
		...
		"gl_Position = u_mvp_matrix * vec4(vPosition,0.0,1.0);" \
	...
	for (int i = 0; i < 2; i++)
	{
		fprintf(gpLogFile, "tangent value is %f\n", tangent[i]);
	}
	vec2 normal = normalize(vec2(tangent[1], -tangent[0]));
	float length = 2;
	...
	{
		fprintf(gpLogFile, "The value of the count is %f\n", randArray[i]);
	}
	...
	for (int i = 0; i < 2; i++)
	{
		for (int j = 0; j <= 1; j++)
		{
			...
			if (randArray[j] > randArray[j + 1])
			{
				...
				{
					buffer = randArray[j];
					//fprintf(gpLogFile,"Value of buffer is %d\n", buffer);
					randArray[j] = randArray[j + 1];
					//	fprintf(gpLogFile,"VALUE OF Number of j is %d\n", randArray[j]);
					randArray[j + 1] = buffer;
					//fprintf(gpLogFile,"VALUE OF Number of ANOTHER NUMBER is %d\n", randArray[j + 1]);
				}
			}
	...
	for (int i = 0; i < count; i++)
	{
		fprintf(gpLogFile, "The NEW  value of the count is %f\n", randArray[i]);
	}
	vec2 A;
	vec2 B;
	...
	int j = 0;
	const float Sway = 80;
	const float Jaggedness = 30 / Sway;
	vec2 prevPoint = source;
	float prevDisplacement = 0;
	for (int i = 1; i < 3; i++)
	{
		
		float pos = randArray[i];
		float scale = (length * Jaggedness) * (pos - randArray[i - 1]);

		float envelope = pos > 0.95f ? 20 * (1 - pos) : 1;
		float displacement = printRandoms2(count);
		displacement -= (displacement - prevDisplacement) * (1 - scale);
		displacement *= envelope;

		vec2 point = source + pos * tangent + displacement * normal;
		vertices[j] = prevPoint[0];
		vertices[j + 1] = prevPoint[1];

		vertices[j + 2] = point[0];
		vertices[j + 3] = point[1];
		j = j + 4;
		prevPoint = point;
		prevDisplacement = displacement;
		
	}
	fprintf(gpLogFile, "Value of j is %d", j);
    vertices[j + 1] = prevPoint[0];
	vertices [j + 2] = prevPoint[1];
	vertices [j + 3] = dest[0];
	vertices [j + 4] = dest[1];
	...
	for (int i = 0; i < 12; i++)
	{
		fprintf(gpLogFile, "The final points A are %f\n", vertices[i]);
