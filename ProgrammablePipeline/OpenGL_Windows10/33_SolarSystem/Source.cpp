...
int initialize(void)
{
	...
	const GLchar *vertexShaderSourceCode =
		...
		"uniform mat4 u_mvprojectionMatrix;" \
		
		"void main(void)" \
		"{" \
		"outColor =  vColor;" \
		"gl_Position = u_mvprojectionMatrix * vPosition;" \
		"}";
    ...
    MVprojectionUniform  = glGetUniformLocation(gShaderProgramObject, "u_mvprojectionMatrix");
...
void display(void)
{
	...
	if (gbLighting == 1)
	{
		...
		glUniform3f(KDUniform, 0.25, 0.25, 0.25);
	...
	viewMatrix = lookat(CameraPos, CameraFocus, CameraUp);
	//push(modelMatrix);
	/*modelViewMatrix *= rotate(00.0f, 0.0f, 0.0f);
	push(modelViewMatrix);*/
	modelViewMatrix =  viewMatrix *modelMatrix;

	modelViewProjectionMatrix = perspectiveProjectionMatrix * modelViewMatrix;
	//perspectiveProjectionMatrix *= modelMatrix * viewMatrix;
	//push(modelViewProjectionMatrix);
	
	
	glUniformMatrix4fv(MVprojectionUniform, 1, GL_FALSE,modelViewProjectionMatrix);
	
	
	//glUniformMatrix4fv(modelViewProjectionUniform, 1, GL_FALSE, modelViewProjectionMatrix);
	...
	modelMatrix *= rotate(0.0f, (float)year, 0.0f);
	modelMatrix *= translate(1.5f, 0.0f, 0.0f);
	//modelMatrix *= rotate(90.0f, 0.0f, 0.0f);
	modelMatrix *= rotate(00.0f, (float)day,0.0f);
	...
	//push(modelMatrix);
	modelMatrix = peek();
	modelMatrix *= rotate(0.0f, (float)day, 0.0f);
	modelMatrix *= translate(0.8f, 0.0f, 0.0f);
//	modelMatrix *= rotate(90.0f, 0.0f, 0.0f);
	modelMatrix *= rotate(0.0f, 0.0f,(float)moonrevo);
//	modelMatrix *= rotate(90.0f, 0.0f,0.0f);

	//modelMatrix *= translate(0.6f, 0.0f, 0.0f);
	modelMatrix *= scale(0.40f, 0.40f, 0.40f);
