...
void launchCudaKernel(float4 *,unsigned int  , unsigned int  ,float, uchar4 * );
...
void display(void)
{
	...
	if (bOnGPU == true)
	{
		...
		error = cudaGraphicsResourceGetMappedPointer((void **)&pColor, &byteCount1, graphicsResourceColor);
		if (error != cudaSuccess)
		{
			fprintf(gpLogFile, "STEP 2 FAILED\n");
		}
		launchCudaKernel(pPos, gMesh_Width, gMesh_Height, animationTime,pColor);
	...
	else
	{
		...
		if(k == 1)
		glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE64 * sizeof(float), pos, GL_DYNAMIC_DRAW);
		if (k == 2)
			glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE128 * sizeof(float), pos, GL_DYNAMIC_DRAW);
		if (k == 3)
			glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE256 * sizeof(float), pos, GL_DYNAMIC_DRAW);
		if (k == 4)
			glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE512 * sizeof(float), pos, GL_DYNAMIC_DRAW);
		if (k == 5)
			glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE * sizeof(float), pos, GL_DYNAMIC_DRAW);
		if (k == 6)
			glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE2048 * sizeof(float), pos, GL_DYNAMIC_DRAW);
		if (k == 7)
			glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE4096 * sizeof(float), pos, GL_DYNAMIC_DRAW);
		glBindBuffer(GL_ARRAY_BUFFER, 0);

		glBindBuffer(GL_ARRAY_BUFFER, vbo_Color);
		glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE * sizeof(float), posColor, GL_DYNAMIC_DRAW);
		if (k == 1)
			glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE64 * sizeof(float), posColor, GL_DYNAMIC_DRAW);
		if (k == 2)
			glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE128 * sizeof(float), posColor, GL_DYNAMIC_DRAW);
		if (k == 3)
			glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE256 * sizeof(float), posColor, GL_DYNAMIC_DRAW);
		if (k == 4)
			glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE512 * sizeof(float), posColor, GL_DYNAMIC_DRAW);
		if (k == 5)
			glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE * sizeof(float), posColor, GL_DYNAMIC_DRAW);
		if (k == 6)
			glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE2048 * sizeof(float), posColor, GL_DYNAMIC_DRAW);
		if (k == 7)
			glBufferData(GL_ARRAY_BUFFER, MYARRAYSIZE4096 * sizeof(float), posColor, GL_DYNAMIC_DRAW);
		glBindBuffer(GL_ARRAY_BUFFER, 0);
