...
int initialize(void)
{
	...
	const GLchar *geometryShaderSource =
		...
		"float randX =  rand(vec2(gl_InvocationID,0.0));" \
		"float randY = -10.0 * rand(vec2(gl_InvocationID,0.0));" \
		...
		"gl_Position = u_mvp_matrix * (gl_in[i].gl_Position + vec4(0.0,1.0,0.0,0.0) + vec4(randX,randY,randZ,0.0));" \
		"EmitVertex();" \

		"fcolor = gs_in[1].color;" \
		"gl_Position = u_mvp_matrix * (gl_in[i].gl_Position + vec4(-1.0,-1.0,0.0,0.0) + vec4(randX,randY,randZ,0.0));" \
		"EmitVertex();" \
