...
//Light 1
GLuint LDUniformL1PerFragment;
GLuint LAUniformL1PerFragment;
GLuint LSUniformL1PerFragment;
GLuint LightPositionUniformL1PerFragment;

//Light 2
GLuint LDUniformL2PerFragment;
GLuint LAUniformL2PerFragment;
GLuint LSUniformL2PerFragment;
GLuint LightPositionUniformL2PerFragment;
...
int initialize(void)
{
	...
	const GLchar *vertexShaderSourceCodePerFragment =
		...
		"out vec3 light_directionL1;" \
		"out vec3 light_directionL2;" \
		"out vec3 viewVector;" \
		"uniform vec4 u_LightPosition;" \
		"uniform vec4 u_LightPositionL1;" \
		"uniform vec4 u_LightPositionL2;" \
		...
		"light_directionL1 = vec3(u_LightPositionL1 - eye_coordinates);" \
		"light_directionL2= vec3(u_LightPositionL2 - eye_coordinates);" \
	...
	const GLchar *fragmentShaderSourceCodePerFragment =
		...
		"in vec3 light_directionL1;" \
		"in vec3 light_directionL2;" \
		"in vec3 viewVector;" \
		...
		"vec3 normalizedLightDirectionL1 = normalize(light_directionL1);" \
		"float tNormal_dot_lightDirectionL1 = max(dot(normalizedLightDirectionL1,normalizedTNorm),0.0);" \
		"vec3 reflectionVectorL1 = reflect(-normalizedLightDirectionL1,normalizedTNorm);" \



		"vec3 normalizedLightDirectionL2 = normalize(light_directionL2);" \
		"float tNormal_dot_lightDirectionL2 = max(dot(normalizedLightDirectionL2,normalizedTNorm),0.0);" \
		"vec3 reflectionVectorL2 = reflect(-normalizedLightDirectionL2,normalizedTNorm);" \
		...
		"vec3 ambientL1 = u_LAUniformL1 * u_KAUniform;" \
		"vec3 diffuseL1 = u_LDUniformL1 * u_KDUniform * tNormal_dot_lightDirectionL1;" \
		"vec3 specularL1 = u_LSUniformL1 * u_KSUniform * pow(max(dot(reflectionVectorL1,normalizedViewVector),0.0),u_MaterialShininess);" \

		"vec3 ambientL2 = u_LAUniformL2 * u_KAUniform;" \
		"vec3 diffuseL2= u_LDUniformL2 * u_KDUniform * tNormal_dot_lightDirectionL2;" \
		"vec3 specularL2 = u_LSUniformL2 * u_KSUniform * pow(max(dot(reflectionVectorL2,normalizedViewVector),0.0),u_MaterialShininess);" \
	...
	LDUniformPerFragment = glGetUniformLocation(gShaderProgramObject, "u_LDUniform");
	LAUniformPerFragment = glGetUniformLocation(gShaderProgramObject, "u_LAUniform");
	LSUniformPerFragment = glGetUniformLocation(gShaderProgramObject, "u_LSUniform");

	LightPositionUniformPerFragment = glGetUniformLocation(gShaderProgramObject, "u_LightPosition");
	
	LDUniformL1PerFragment = glGetUniformLocation(gShaderProgramObject, "u_LDUniformL1");
	LAUniformL1PerFragment = glGetUniformLocation(gShaderProgramObject, "u_LAUniformL1");
	LSUniformL1PerFragment = glGetUniformLocation(gShaderProgramObject, "u_LSUniformL1");


	LightPositionUniformL1PerFragment = glGetUniformLocation(gShaderProgramObject, "u_LightPositionL1");

	LDUniformL2PerFragment = glGetUniformLocation(gShaderProgramObject, "u_LDUniformL2");
	LAUniformL2PerFragment = glGetUniformLocation(gShaderProgramObject, "u_LAUniformL2");
	LSUniformL2PerFragment = glGetUniformLocation(gShaderProgramObject, "u_LSUniformL2");
	LightPositionUniformL2PerFragment = glGetUniformLocation(gShaderProgramObject, "u_LightPositionL2");
...
void display(void)
{
	...
	GreenLightRotate = GreenLightRotate + 0.005f;;
	RedLightRotate = RedLightRotate + 0.005f;
	BlueLightRotate = BlueLightRotate + 0.005f;
}
...
void PerVertexLighting()
{
	...
	if (gbLighting == 1)
	{
		...
		lights[0].Position[0] = 0.0f;
		lights[0].Position[1] = sin(RedLightRotate) * 100.0f;
		lights[0].Position[2] = cos(RedLightRotate) * 100.0f - 3.0f;
		...
		lights[2].Position[0] = sin(BlueLightRotate) * 100.0f;
		lights[2].Position[1] =cos(BlueLightRotate) * 100.0f;
		lights[2].Position[2] = 0.0f;
...
void PerFragmentLighting()
{
	...
	if (gbLighting == 1)
	{
		...
		glUniform3fv(LDUniformPerFragment, 1, lights[0].Diffuse);
		glUniform3fv(LAUniformPerFragment, 1, lights[0].Ambient);
		glUniform3fv(LSUniformPerFragment, 1, lights[0].Specular);

		glUniform3fv(LDUniformL1PerFragment, 1, lights[1].Diffuse);
		glUniform3fv(LAUniformL1PerFragment, 1, lights[1].Ambient);
		glUniform3fv(LSUniformL1PerFragment, 1, lights[1].Specular);


		glUniform3fv(LDUniformL2PerFragment, 1, lights[2].Diffuse);
		glUniform3fv(LAUniformL2PerFragment, 1, lights[2].Ambient);
		glUniform3fv(LSUniformL2PerFragment, 1, lights[2].Specular);
		...
		glUniform4fv(LightPositionUniformPerFragment, 1, lights[0].Position);

		lights[1].Position[0] = sin(GreenLightRotate) * 100.0f;
		lights[1].Position[1] = 0.0f;
		lights[1].Position[2] = cos(GreenLightRotate) * 100.0f - 3.0f;

		glUniform4fv(LightPositionUniformL1PerFragment, 1, lights[1].Position);

		lights[2].Position[0] = sin(BlueLightRotate) * 100.0f;
		lights[2].Position[1] = cos(BlueLightRotate) * 100.0f;
		lights[2].Position[2] = 0.0f;
		glUniform4fv(LightPositionUniformL2PerFragment, 1, lights[2].Position);
	}
	else
	{
		glUniform1i(LKeyPressed, 0);
