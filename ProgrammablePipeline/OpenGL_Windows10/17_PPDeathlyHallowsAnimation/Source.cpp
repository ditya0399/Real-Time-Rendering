...
//Variables for animation of DeathlyHallows
GLfloat Trianglex = 4.0f;
GLfloat Triangley = -4.0f;
GLfloat TriangleRotate = 0.0f;
GLfloat TriangleCatalyst = 4.0f;
GLfloat Circlex = -4.0f;
GLfloat Circley = -4.0f;
GLfloat CircleRotate = 0.0f;
GLfloat CircleCatalyst = -4.0f;
GLfloat ElderWandy = 3.0f;
...
GLuint vao_ElderWand;
GLuint vbo_ElderWand;
...
int initialize(void)
{
	...
	Vertices = DrawCircle(&CircleVertices);
	...
	glBufferData(GL_ARRAY_BUFFER, 3 * Vertices * sizeof(CircleVertices), CircleVertices, GL_STATIC_DRAW);
	glVertexAttribPointer(AMC_ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(AMC_ATTRIBUTE_POSITION);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	glBindVertexArray(0);
	...
	////// ELDER WAND /////////
	glGenVertexArrays(1, &vao_ElderWand);
	glBindVertexArray(vao_ElderWand);
	glGenBuffers(1, &vbo_ElderWand);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_ElderWand);
	glBufferData(GL_ARRAY_BUFFER, sizeof(ElderWand), ElderWand, GL_STATIC_DRAW);
	glVertexAttribPointer(AMC_ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(AMC_ATTRIBUTE_POSITION);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	glBindVertexArray(0);
...
void display(void)
{
	...
	modelViewMatrix = translate(Trianglex, Triangley, -8.5f);
	RotationMatrix = rotate(TriangleRotate, 0.0f, 1.0f, 0.0f);
	...
	modelViewProjectionMatrix = mat4::identity();
	RotationMatrix = mat4::identity();
	RotationMatrix = rotate(CircleRotate, 0.0f, 1.0f, 0.0f);
	modelViewMatrix = translate(Circlex, Circley, -8.5f);
	modelViewMatrix = modelViewMatrix * RotationMatrix;
	modelViewProjectionMatrix = perspectiveProjectionMatrix * modelViewMatrix;
	...
	// draw necessary scene
	glDrawArrays(GL_POINTS, 0, Vertices * 1);
	glBindVertexArray(0);

	//// ElderWand
	modelViewMatrix = mat4::identity();
	modelViewProjectionMatrix = mat4::identity();
	modelViewMatrix = translate(0.0f, ElderWandy, -8.5f);
	modelViewProjectionMatrix = perspectiveProjectionMatrix * modelViewMatrix;
	...
	if (Trianglex >= 0.0f)
	{
		Trianglex = Trianglex - 0.002f;
	}
	if (Triangley <= 0.1f)
	{
		Triangley = Triangley + 0.002f;
	}
	if (TriangleCatalyst >= -0.14f)
	{
		TriangleCatalyst = TriangleCatalyst - 0.002f;
		TriangleRotate = TriangleRotate + 1.3f;
	}

	if (Circlex <= 0.0f)
	{

		Circlex = Circlex + 0.002f;
	}
	if (Circley <= 0.1f)
	{
		Circley = Circley + 0.002f;
	}
	if (CircleCatalyst <= 0.13f)
	{
		CircleCatalyst = CircleCatalyst + 0.002f;
		CircleRotate = CircleRotate + 1.3f;
	}
	if (ElderWandy >= 0.1f)
	{
		ElderWandy = ElderWandy - 0.0014f;
	}
}
...
int DrawCircle(GLfloat **Vert)
{

	int n = 0;
	//int k = 0;
	for (GLfloat fAngle1 = 0.0f; fAngle1 < (2.0f * M_PI); fAngle1 = fAngle1 + 0.001f)
	{
		n = n + 1;
	}

	*Vert = (GLfloat *)malloc(n * 3 * sizeof(GLfloat));
	GLfloat *Vertices = *Vert;
	n = 0;
	for (GLfloat fAngle = 0.0f; fAngle < (2.0f * M_PI); fAngle = fAngle + 0.001f)
	{
		Vertices[(3 * n) + 0] = (radius)*(GLfloat)cos(fAngle) + xOffSet1;
		Vertices[(3 * n) + 1] = (radius)*(GLfloat)sin(fAngle) + yOffSet2;
		Vertices[(3 * n) + 2] = 0.0f;
		n = n + 1;
	}
	//fprintf(gpLogFile, "vALUE OF N IS %d\n", n);
